...
module.exports = function notFound (data, options) {
  ...
  if (sails.config.environment === 'production') {
	...
	if(typeof req.session.authenticated != 'undefined' && req.session.authenticated) {
		res.setLocale(req.session.lang.toLowerCase());
	}
	else {
		res.setLocale('ro');
	}
  ...
  // work, just send JSON.
  if (options.view) {
    return res.view(options.view, { layout: '404', data: data });
  }

  // If no second argument provided, try to serve the default view,
  // but fall back to sending JSON(P) if any errors occur.
  else return res.view('404', { layout: '404', data: data }, function (err, html) {
    ...
    if (err) {
      ...
      return res.view('404',{ layout: '404',data: data});
    }
