...
module.exports.sockets = {
  ...
  ***************************************************************************/
  onConnect: function(session, socket) {
	socket.sid = typeof socket.handshake != 'undefined' ? socket.handshake.sessionID : null;
	if(socket.sid && socket.handshake.session.authenticated && typeof socket.handshake.session.currentUser != 'undefined' && typeof sails.storage.userSessions[socket.handshake.session.currentUser.ID] != 'undefined' && sails.storage.userSessions[socket.handshake.session.currentUser.ID].sid == socket.sid) {
		sails.storage.userSessions[socket.handshake.session.currentUser.ID].lastUpdate = Date.now();
		sails.storage.userSessions[socket.handshake.session.currentUser.ID].socket = socket.id;
		//console.log(socket.handshake.session.currentUser.ID);
		//console.log(sails.storage.userSessions[socket.handshake.session.currentUser.ID]);
	}
  },
  ...
  onDisconnect: function(session, socket) {
	if(typeof socket.sid != 'undefined') {
		...
		for(var i in sails.storage.userSessions) {
			if(sails.storage.userSessions[i].sid == socket.sid) {
				currentUser = i;
			}
		}
		if(currentUser>-1 && typeof sails.storage.userSessions[currentUser]) {
			sails.storage.userSessions[currentUser].socket = null;
		}
		//&& typeof sails.storage.userSessions[sails.storage.users[socket.sid].ID] != 'undefined' && sails.storage.userSessions[sails.storage.users[socket.sid].ID] == socket.sid) {
		//delete sails.storage.userSessions[sails.storage.users[socket.sid].ID];
	}
  },
